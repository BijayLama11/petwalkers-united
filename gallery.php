...
<head>
  ...
  <meta name="description" content="Media gallery of our walks and sits with interactive previews." />
...
<body>
  ...
  <main id="main">
      ...
      <?php if (count($galleryImages) > 0): ?>
        <ul class="thumb-grid" role="list">
          <?php foreach ($galleryImages as $image): ?>
            <li>
              <button class="thumb" data-full="<?php echo htmlspecialchars($image['image_url']); ?>"
    ...
    <!-- Modal viewer -->
    <div class="lightbox" role="dialog" aria-modal="true" aria-labelledby="lb-title" hidden>
      <div class="lightbox-backdrop" data-close></div>
      <figure class="lightbox-content" role="group" aria-label="Expanded media">
        <img id="lb-img" src="" alt="Expanded gallery image" />
        <figcaption id="lb-title">Caption</figcaption>
        <button class="lb-close" type="button" aria-label="Close viewer" data-close>
          &times;
        </button>
      </figure>
    </div>
